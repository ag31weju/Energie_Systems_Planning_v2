...
def get_abstract_pyomo_model(fix_capacities=False):
    ...
    # Power Variables
    model.Pg = pyo.Var(
        model.Ug, model.T, within=pyo.NonNegativeReals
    )  # Generation #send to frontend
    model.Pd = pyo.Var(
        model.Uc, model.T, within=pyo.NonNegativeReals
    )  # Demand      #send to frontend
    model.nSPd = pyo.Var(
        model.Uc, model.T, within=pyo.NonNegativeReals
    ...
    model.demand_profile_eq = pyo.Constraint(
        model.Uc, model.T, rule=demand_profile_rule
    )
    ...
    model.availability_profile_eq = pyo.Constraint(
        model.Ug, model.T, rule=availability_profile_rule
    )
...
if __name__ == "__main__":
    ...
    print("Unmet Demand: ", get_variable_value(instance, "nSPd"))
    print("Generation Capacity: ", get_variable_value(instance, "Cg"))
    print("Power Injection: ", get_variable_value(instance, "Pi"))
